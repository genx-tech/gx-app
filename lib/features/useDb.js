...
module.exports = {
  ...
  load_: async (app, dbRefs) => {
    ...
    console.log('db feature', app.name);

    app.db = (schemaName, originApp) => {
      console.log('app.db', schemaName, originApp ? originApp.name : null);
      ...
      if (schemaInfo.fromLib) {
        ...
        console.log('origin app', app.name);
        ...
        console.log('ofiejfioejifajofeojfoewjafjo');

        const Db = require(path.join(modelPath, naming.pascalCase(schemaName)));

        console.log('in lib', (originApp || app).name);
