...
class ServiceContainer extends EventEmitter {
  ...
  async _loadFeatures_() {
    ...
    let configStageFeatures = []; // load features
    ...
    _.forOwn(this.config, (featureOptions, name) => {
      ...
      } catch (err) {//ignore the first trial                
      }
    ...
    if (configStageFeatures.length > 0) {
      ...
      await this._loadFeatureGroup_(configStageFeatures, Feature.CONF); //reload all features if any type of configuration feature exists
  ...
  _loadFeature(feature) {
    ...
    if (this._featureRegistry.hasOwnProperty(feature)) {
      ...
      let searchingPath = this._featureRegistry["*"]; //reverse fallback stack
